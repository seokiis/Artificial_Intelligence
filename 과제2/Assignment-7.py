...
embed = Sequential()
embed.add(Embedding(input_dim=dic_siz,
          output_dim=embed_space_dim, input_length=sample_siz))
embed.add(LSTM(units=32))
embed.add(Dense(1, activation='sigmoid'))
embed.compile(loss='binary_crossentropy',
...
for train_idxs, valid_idxs in kfold.split(x_train):
    ...
    # 모델 학습
    hist = embed.fit(x_train_fold, y_train_fold, epochs=5, batch_size=64, validation_data=(
        x_valid_fold, y_valid_fold), verbose=2, callbacks=[early])
